...
namespace myportio_driver
{
	// Device name
	constexpr const wchar_t* DEVICE_NAME = L"\\\\.\\MyPortIODev";

	// IOCTLs from Ghidra RE of MyPortIO_x64.sys
	// These do MmMapIoSpace, read/write 4 bytes, then MmUnmapIoSpace
	constexpr ULONG32 IOCTL_READ_PHYS = 0x9c406680;   // Read 4 bytes from physical address
	constexpr ULONG32 IOCTL_WRITE_PHYS = 0x9c40a684;  // Write 4 bytes to physical address

	// Structure for READ: 12 bytes input, output overwrites first 4 bytes
	#pragma pack(push, 1)
	struct READ_PHYS_REQUEST {
		uint64_t PhysicalAddress;  // Offset 0
		uint32_t Size;            // Offset 8 (size to read, always 4)
	};

	// Structure for WRITE: 16 bytes minimum (12 header + 4 value)
	struct WRITE_PHYS_REQUEST {
		uint64_t PhysicalAddress;  // Offset 0
		uint32_t Size;            // Offset 8 (size of data to write)
		uint32_t Value;           // Offset 12 (data to write)
	};
	#pragma pack(pop)
	...
	// Core physical memory operations (4 bytes at a time)
	bool ReadPhysDword(uint64_t phys_addr, uint32_t* out_value);
	bool WritePhysDword(uint64_t phys_addr, uint32_t value);

	// Bulk physical memory operations (loops in 4-byte chunks)
