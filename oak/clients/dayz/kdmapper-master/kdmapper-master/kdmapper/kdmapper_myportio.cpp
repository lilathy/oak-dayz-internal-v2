// kdmapper using MyPortIO driver instead of Intel driver
#include "include/kdmapper.hpp"
#include <Windows.h>
#include <iostream>

#include "include/utils.hpp"
#include "include/myportio_driver.hpp"  // Using MyPortIO instead of intel_driver
#include "include/nt.hpp"
#include "include/portable_executable.hpp"

void RelocateImageByDelta_MyPortIO(portable_executable::vec_relocs relocs, const ULONG64 delta) {
...
bool FixSecurityCookie_MyPortIO(void* local_image, ULONG64 kernel_image_base)
{
...
bool ResolveImports_MyPortIO(portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		...
		for (auto& current_function_data : current_import.function_datas) {
			ULONG64 function_address = myportio_driver::GetKernelModuleExport(Module, current_function_data.name);

			if (!function_address) {
				if (Module != myportio_driver::ntoskrnlAddr) {
					function_address = myportio_driver::GetKernelModuleExport(myportio_driver::ntoskrnlAddr, current_function_data.name);
					if (!function_address) {
#if !defined(DISABLE_OUTPUT)
...
ULONG64 MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, kdmapper::AllocationMode mode, bool PassAllocationAddressAsFirstParam, kdmapper::mapCallback callback, NTSTATUS* exitCode) {
	...
	{
		kernel_image_base = myportio_driver::MmAllocateIndependentPagesEx(image_size);
	}
	else {
		kernel_image_base = myportio_driver::AllocatePool(nt::POOL_TYPE::NonPagedPool, image_size);
	}
	...
	do {
		...
		RelocateImageByDelta_MyPortIO(portable_executable::GetRelocs(local_image_base), kernel_image_base - nt_headers->OptionalHeader.ImageBase);

		if (!FixSecurityCookie_MyPortIO(local_image_base, kernel_image_base))
		{
			kdmLog(L"[-] Failed to fix cookie" << std::endl);
			return 0;
		}

		if (!ResolveImports_MyPortIO(portable_executable::GetImports(local_image_base))) {
			kdmLog(L"[-] Failed to resolve imports" << std::endl);
			kernel_image_base = realBase;
			break;
		}

		if (!myportio_driver::WriteMemory(realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
		...
		if (mode == kdmapper::AllocationMode::AllocateIndependentPages)
		{
			for (int i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
				...
				myportio_driver::MmSetPageProtection(secAddr, secSize, prot);
			}
		}
		...
		NTSTATUS status = 0;
		if (!myportio_driver::CallKernelFunction(&status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
		...
		if (free) {
			...
			if (mode == kdmapper::AllocationMode::AllocateIndependentPages)
			{
				free_status = myportio_driver::MmFreeIndependentPages(realBase, image_size);
			}
			else {
				free_status = myportio_driver::FreePool(realBase);
			}
	...
	if (mode == kdmapper::AllocationMode::AllocateIndependentPages)
	{
		free_status = myportio_driver::MmFreeIndependentPages(kernel_image_base, image_size);
	}
	else {
		free_status = myportio_driver::FreePool(kernel_image_base);
	}
