...
#include "imgui_dx11_shaders.h" // precompiled VS/PS — no runtime D3DCompile

// DirectX
#include <stdio.h>
#include <d3d11.h>
// d3dcompiler intentionally NOT used (manual-map crashes in D3DCompile)
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Oak: use precompiled bytecode (imgui_dx11_shaders.h) — D3DCompile hard-crashes under kernel manual-map.

    // Create the vertex shader
    {
        if (bd->pd3dDevice->CreateVertexShader((const void*)g_ImGuiDX11VS, g_ImGuiDX11VS_size, nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, (const void*)g_ImGuiDX11VS, g_ImGuiDX11VS_size, &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader((const void*)g_ImGuiDX11PS, g_ImGuiDX11PS_size, nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
